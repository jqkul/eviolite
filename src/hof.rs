...
use crate::{Solution, Cached, select::{rank_nondominated, utils::retain_indices}, fitness::MultiObjective};
...
impl<T> HallOfFame<T> for BestN<T> where T: Solution, T::Fitness: Into<f64> {
    fn record(&mut self, generation: &[Cached<T>]) {
...
impl<T> Debug for BestN<T> where T: Solution, Cached<T>: Debug {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_list().entries(self.best.iter()).finish()
    }
}

impl<T, F> BestN<T> where T: Solution<Fitness = F>, F: Into<f64> {
...
#[derive(Clone)]
pub struct BestPareto<T, const M: usize> where T: Solution<Fitness = MultiObjective<M>> {
    front: Vec<Cached<T>>
}

impl<T, const M: usize> BestPareto<T, M> where T: Solution<Fitness = MultiObjective<M>> {
...
impl<T, const M: usize> Default for BestPareto<T, M> where T: Solution<Fitness = MultiObjective<M>> {
    fn default() -> Self {
        BestPareto {
            front: Vec::new()
        }
...
impl<T, const M: usize> HallOfFame<T> for BestPareto<T, M> where T: Solution<Fitness = MultiObjective<M>> {
    fn record(&mut self, generation: &[Cached<T>]) {
        ...
        let indices = (0..self.front.len()).filter(|i| pareto2.ranks[*i] == 0).collect();
        retain_indices(&mut self.front, indices);
    }
}

impl<T, const M: usize> IntoIterator for BestPareto<T, M> where T: Solution<Fitness = MultiObjective<M>> {
    type Item = Cached<T>;
    type IntoIter = IntoIter<T>;
    fn into_iter(self) -> Self::IntoIter {
        IntoIter {
...
impl<T, const M: usize> Debug for BestPareto<T, M> where T: Solution<Fitness = MultiObjective<M>>, Cached<T>: Debug {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
