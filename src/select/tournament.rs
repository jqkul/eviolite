...
impl Tournament {
    ...
    pub fn round<'a, T: Solution>(&self, population: &'a [Cached<T>], cmp: impl Fn(&Cached<T>, &Cached<T>) -> Ordering) -> &'a Cached<T> {
    ...
    F: Into<f64>
{
    fn select(&self, n_rounds: usize, pop: &mut Vec<Cached<T>>) {
        ...
        for _ in 0..n_rounds {
            winners.push(self.round_idx(
                pop,
                |a, b| f64::partial_cmp(&a.evaluate().into(), &b.evaluate().into()).unwrap()
            ));
        }
...
impl Tournament {
    pub(crate) fn round_idx<T: Solution>(&self, pop: &[Cached<T>], cmp: impl Fn(&Cached<T>, &Cached<T>) -> Ordering) -> usize {
